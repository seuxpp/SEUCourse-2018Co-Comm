...
%     INTERVAL = 0.5;	% SNR interval
    ...
    POW = 1;        % without cooperation,Source send signals directly to the Destination with full power
    Monte_MAX=2*10^2;    % the times of Monte Carlo,Limited to the computer configuration level, select the number to 10
...
%     H_rd = RayleighCH( 1 );     % between Relay station and Destination
    H_sd = 0.8035 - 0.3889i;     % between Source and Destination
    H_sr = 0.2147 + 1.1708i;  	% between Source and Relay station
    H_rd = 0.0834 - 0.9688i;     % between Relay station and Destination
...
for SNR_dB=MIN_SNR_dB:INTERVAL:MAX_SNR_dB
    ...
    for tries=0:Monte_MAX
        ...
        POW_S2 = 1-POW_DIV;      % Signal power2
    ...
    %02:With Fixed Amplify-and-Forward relaying protocol
        ...
        y_combine_AF = Mrc( H_sd,H_sr,H_rd,beta,POW_S,POW_N,POW_S2,POW_N,y_sd,y_rd);  % MRC
    ...
    %03:With Fixed Decode-and-Forward relaying protocol
        x_DF = DF(M,y_sr,x);
        y_rd = awgn( sqrt(POW_S2)*H_rd * x_DF, SNR_dB, 'measured');
        y_combine_DF = Mrc( H_sd,H_rd,POW_S,POW_N,POW_S2,POW_N,y_sd,y_rd);
...
%     theo_ber_SD(snrcount) = Theo_ber(SNR_dB);
    theo_ber_SD(snrcount) = Theo_ber(sig);
    theo_ber_AF(snrcount) = Theo_ber(H_sd,H_sr,H_rd,POW_S,POW_N,POW_S2,POW_N);
    theo_ber_DF(snrcount) = Theo_ber(H_sd,H_rd,POW_S,POW_N,POW_S2,POW_N);
    
    disp('SNR=');disp(SNR_dB);%disp 控制显示函数
