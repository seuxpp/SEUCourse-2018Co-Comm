...
signal(1).modulation_type='QPSK' %单个链路功率p发送
...
for iSNR=1:size(SNR,2)
    ...
    channel(3).noise(1).SNR=SNR(iSNR);
     for it=1:nr_of_iterations
          rx(1)=rx_reset(rx(1));
          [channel(1),rx(1)]=add_channel_effect(channel(1),rx(1),sqrt(1/2)*signal.symbol_sequence);
          %rx(1)=rx_correct_phaseshift(rx(1),channel(1).attenuation.phi);
          [channel(2),rx(1)]=add_channel_effect_noNoise(channel(2),rx(1),sqrt(1/2)*signal.symbol_sequence);
