...
for SNR_dB=MIN_SNR_dB:INTERVAL:MAX_SNR_dB
    ...
    %02:With Fixed Amplify-and-Forward relaying protocol
       ...
       % y_combine_AF = y_sd+y_rd;  % MRC
        y_AF_sd = demodulate(modem.pskdemod(M),y_sd); % After demodulate, Destinationthe gains the signal 'y_AF' 
        y_AF_rd = demodulate(modem.pskdemod(M),y_rd); 
        if Act_ber(x,y_AF_sd)>Act_ber(x,y_AF_rd)
           err_num_AF = err_num_AF + Act_ber(x,y_AF_rd);       % wrong number of bits with AF
        else
           err_num_AF = err_num_AF + Act_ber(x,y_AF_sd); 
        end
    
    %03:With Fixed Decode-and-Forward relaying protocol
        x_DF = DF(M,y_sr,x);
        y_rd = awgn( sqrt(POW_DIV)*H_rd * x_DF, SNR_dB, 'measured');
   %     y_combine_DF = y_sd+y_rd;
        y_DF_sd = demodulate(modem.pskdemod(M),y_sd);
        y_DF_rd = demodulate(modem.pskdemod(M),y_rd);
        if Act_ber(x,y_DF_sd)>Act_ber(x,y_DF_rd)
           err_num_DF = err_num_DF + Act_ber(x,y_DF_rd);       % wrong number of bits with DF
        else
           err_num_DF = err_num_DF + Act_ber(x,y_DF_sd); 
        end
