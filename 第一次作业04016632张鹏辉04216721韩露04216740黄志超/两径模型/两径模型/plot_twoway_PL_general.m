...
Exp=[2 3 6];
for k=1:3
    y_Free(k,:)=PL_twoway_free(distance,Gt(k),Gr(k));
   y_logdist(k,:)=PL_twoway_logdist_or_norm(distance,d0,Exp(k));
   y_lognorm(k,:)=PL_twoway_logdist_or_norm(distance,d0,Exp(1),sigma);
end
%subplot(131)
figure(1)
